...
export function DeleteForm({ id_professor }: { id_professor: number }) {
  
  const handleFormSubmit = async () => {
    try {
      const res = await DeleteProfessor(`professores/${id_professor}`);

      if (res.status === 200) {
        toast.success(res.message);
      } else {
        toast.error(res.message || "Failed to delete professor.");
      }
    } catch (error) {
